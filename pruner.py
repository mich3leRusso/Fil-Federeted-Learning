...
class Pruner(object):
    ...
    def select_random_weights(self, weights, mask):
        ...
        n_random = weights.numel() // config_parameters["n_experiences"]
    ...
    def ripristinate_weights(self, model, old_model, experience_idx, distillation):
        ...
        for module_idx, (module, old_module) in enumerate(zip(model.modules(), old_model.modules())):
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                ...
                if config_parameters["self_distillation"] and not distillation:#if you have the self distillation process  
    ...
    def most_important_weights_mask(self, weights, mask):
        ...
        # get the number of weights to prune linearly
        n_important = weights.numel() // config_parameters["n_experiences"]
            ...
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
               #put into the device  
                ...
                if not masks_created:
                    ...
                    if module_idx == n_modules-2 and config_parameters["dataset"] != 'CORE50' and not config_parameters["self_distillation"]:
                        subset = torch.zeros_like(self.masks[module_idx])
                        if config_parameters["extra_classes"] > 0:
                            subset[experience_idx * (config_parameters["classes_per_exp"] + config_parameters["extra_classes"]):(experience_idx + 1) * (config_parameters["classes_per_exp"] + config_parameters["extra_classes"]), :] = 1
                        else:
                            subset[experience_idx*config_parameters["classes_per_exp"]:(experience_idx+1)*config_parameters["classes_per_exp"], :] = 1
                        #convert subset to bool
                        subset = subset.to(torch.bool)
                    else:
                        if config_parameters["self_distillation"]:#praticamente mai 
                ...
                if not self_distillation:
                    ...
                    if not config_parameters["weight_sharing"]: #entra qui perchè c'è il weight sharing sempre disattivo 
        ...
        for experience_idx in range(n_tasks):
            ...
            for module_idx, module in enumerate(model.modules()):
                if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
                    ...
                    if module_idx == n_modules-2 and config_parameters["dataset"] != 'CORE50' and not config_parameters["self_distillation"]:
                        subset = torch.zeros_like(self.masks[module_idx])
                        if config_parameters["extra_classes"] > 0: #set the last task predictions
                            subset[experience_idx * (config_parameters["classes_per_exp"] + config_parameters["extra_classes"]):(experience_idx + 1) * (config_parameters["classes_per_exp"] + config_parameters["extra_classes"]), :] = 1
                        else:
                            subset[experience_idx*config_parameters["classes_per_exp"]:(experience_idx+1)*config_parameters["classes_per_exp"], :] = 1
