...
class GatedCifarResNet(nn.Module):
    ...
    def __init__(self, block, depth, channels=3,dropout_rate=0.1,classes_out=100):
        ...
        if config_parameters["dataset"]=='CORE50':
            expansion = 4 # 4*4
        elif config_parameters["dataset"] == 'TinyImageNet':
            expansion = 4
        else:
        ...
        self.fc = GatedLinear(64 * expansion, classes_out+config_parameters["extra_classes"]*config_parameters["n_experiences"], bias=False)
        self.output_mask = {}
        self.exp_idx = -1
        self.bn_weights = {}
        self.dropout = torch.nn.Dropout(p= dropout_rate)
    ...
    def forward(self, x):
        ...
        # # }
        # print(self.exp_idx)
        # print(self.fc(features).shape)
        # print(self.output_mask[self.exp_idx].shape)
        # input()
    ...
    def average_weights(models, module_idx):
        # Accumulate weights without tracking gradients
        with torch.no_grad():
            weights = [list(m.model.modules())[module_idx].weight.data for m in models]
            avg_weights = torch.mean(torch.stack(weights), dim=0)

        return avg_weights


    def aggregate_function(central_model, client_models):
        with torch.no_grad():  # no gradients during aggregation
            for i, m in enumerate(central_model.model.modules()):
                if isinstance(m, (GatedConv2d, GatedLinear, nn.BatchNorm2d)):
                    avg_weights = average_weights(client_models, i)
                    m.weight.data.copy_(avg_weights)  #in-place data copy (safe)
    ...
    def set_output_mask(self, exp_idx, classes_in_this_exp):
        ...
        self.output_mask[exp_idx] = torch.nn.functional.one_hot(torch.tensor(classes_in_this_exp),num_classes=config_parameters["n_classes"]+config_parameters["extra_classes"]*config_parameters["n_experiences"]).sum(dim=0).float().to(config_parameters["device"])
        ...
        for i , (m, m_) in enumerate(zip(central_model.model.modules(), self.modules())):
            ...
            if isinstance(m, GatedConv2d) or isinstance(m ,GatedLinear) or isinstance(m , nn.BatchNorm2d):
                ...
                if hasattr(m, "bias") and m.bias is not None:
                    m_.bias.data.copy_(m.bias.data)  
...
def gresnet32(dropout_rate=0.1):
    """Constructs a ResNet-32 model for CIFAR-10."""
    
    classes_out=config_parameters["n_classes"]
